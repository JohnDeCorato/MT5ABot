...
class EGL:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.egl_db = database.Database('egl.json')
    ...
    # Utility function for asking a yeo/no question
    ...
    # Utility function for asking a multiple choice question
    ...
    # Gives the survey and assigns the member wit hthe appropriate roles
    async def give_survey(self, member):
        survey = self.egl_db.get('survey', {})
        try:
            intro = survey['intro']
        except KeyError:
            intro = "Welcome to the EGL Discord Server. We require taking a brief survey."
    ...
    async def multiple_choice(self, ctx, question: str, *, question_number: int=-1):
        ...
        await self.bot.say('Lets begin setting up the responses for this question. Send `cancel` at any point to quit.')
        while True:
            ...
            if reply.content == 'done':
                if len(responses) >= 2:
                    ...
                    await self.bot.send_message(channel, 'You must have at least two responses for multiple choice.')
            else:
                response = {
    ...
    @survey.command(pass_context=True, hideen=True)
    @is_egl_server()
    @admin_or_bot_owner()
